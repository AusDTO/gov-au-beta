...
RSpec.describe 'node features', :type => :feature do
  ...
  context 'a node with a uuid link' do
    ...
    let(:linking_content) { Fabricate(:content_block, unique_id: 'uuid_cb',
      body: '<a href="/invalid" data-uuid="uuid_nodeA">My link</a>')}
    let!(:linking_node) { Fabricate(:node, name: 'nodeB', uuid:'uuid_nodeB',
      section: root, content_block: linking_content) }
