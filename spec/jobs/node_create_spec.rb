...
describe 'Node Creation' do
  ...
  before(:each) do
    stub_request(:get, 'www.example.com/api/node/1/1')
        .to_return(:headers => {'Content-Type' => 'application/json'},
                   :body => drupal_node)
    stub_request(:post, %r{www.example.com/api/node/result/\d+/1})
        .to_return(:headers => {'Content-Type' => 'application/json'},
  ...
  context 'when given a system error when posting a result' do
    it 'should create a node in this system' do
      stub_request(:get, 'www.example.com/api/node/1/2')
          .to_return(:headers => {'Content-Type' => 'application/json'},
                     :body => drupal_node)
      stub_request(:post, 'www.example.com/api/node/result/1/2').
          to_return(:status => 500, :body => 'System error', :headers => {})
  ...
  context 'when given a node with no parent' do
    it 'should create a node with no parent' do
      ...
      stub_request(:get, 'www.example.com/api/node/2/1')
          .to_return(:headers => {'Content-Type' => 'application/json'},
                     :body => JSON.dump(body))
